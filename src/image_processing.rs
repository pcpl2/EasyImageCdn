        ...
        match image::guess_format(&job.image_data) {
            Ok(format) => {
                if let Some(extension) = format.extensions_str().first() {
                    ...
                    tracing::debug!("Attempting to save original file (format: {:?}) to: {:?}", format, original_output_path);
    
                    fs::write(&original_output_path, &job.image_data)
                        .with_context(|| format!("Failed to write original image to {:?}", original_output_path))?;
    
                    tracing::info!("Successfully saved original file: {:?}", original_output_path);
                } else {
                    tracing::warn!(
    ...
    for (target_width, target_height) in &job.target_resolutions {
       ...
       // let resized_img = img.resize_exact(*target_width, *target_height, imageops::FilterType::Lanczos3);
        ...
        for format in &job.target_formats {
             let filename_base = job.image_id.split(|c| c == '/' || c == '\\').last().unwrap_or(&job.image_id); // Podstawowa nazwa pliku
             let filename = format!(
                 "{}_{}x{}.{}",
                 filename_base,
                 target_width,
            ...
            match format {
                TargetFormat::WebP => {
                    resized_img.save_with_format(&output_path, ImageFormat::WebP)
                        .with_context(|| format!("Failed to save AVIF using 'image' crate to {:?}", output_path))?;
                }
                TargetFormat::Avif => {
                ...
                TargetFormat::Jpeg => {
                    resized_img.save_with_format(&output_path, ImageFormat::Jpeg)
                    .with_context(|| format!("Failed to save AVIF using 'image' crate to {:?}", output_path))?;
                }
            }
